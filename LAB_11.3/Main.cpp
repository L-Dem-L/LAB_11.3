...
bool b_search(Student* S, const int N, const string surname, const double average_evaluation, const int math_grade);
...
void p_sort(Student* S, int N) {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N - i - 1; j++) {
            if ((CalculateAverageGrade(S[j]) > CalculateAverageGrade(S[j + 1])) ||
                (CalculateAverageGrade(S[j]) == CalculateAverageGrade(S[j + 1])) &&
                (S[j].math_grade > S[j + 1].math_grade) ||
                (CalculateAverageGrade(S[j]) == CalculateAverageGrade(S[j + 1])) &&
                (S[j].math_grade == S[j + 1].math_grade) &&
...
int* i_sort(Student* S, const int N) {
    ...
    for (int i = 1; i < N; i++) {
        value = I[i];
        for (j = i - 1; (CalculateAverageGrade(S[j]) > CalculateAverageGrade(S[j + 1])) ||
            (CalculateAverageGrade(S[j]) == CalculateAverageGrade(S[j + 1])) &&
            (S[j].math_grade > S[j + 1].math_grade) ||
            (CalculateAverageGrade(S[j]) == CalculateAverageGrade(S[j + 1])) &&
            (S[j].math_grade == S[j + 1].math_grade) &&
...
bool b_search(Student* S, const int N, const string surname, const double average_evaluation, const int math_grade) {
    ...
    do {
        m = (L + R) / 2;
        if ((S[m].surname == surname && CalculateAverageGrade(S[m]) == average_evaluation && S[m].math_grade == math_grade)) {
            return true;
        }

        if ((S[m].math_grade < math_grade) ||
            (S[m].math_grade == math_grade && CalculateAverageGrade(S[m]) < average_evaluation) ||
            (S[m].math_grade == math_grade && CalculateAverageGrade(S[m]) == average_evaluation && S[m].surname < surname)) {
            L = m + 1;
        }
        else {
            R = m - 1;
